...
4. Recommend next command based on detected docs and feature readiness.
	...
	- If the request asks to create or evolve feature specs, recommend `domainspec-orchestrate "spec <feature>"` and note that orchestrator runs delegated `domainspec-spec-feature`, delegated task refresh/create via `domainspec-plan-phase-bridge <feature> --mode native`, then delegated `domainspec-sync-user-stories`.
	- If the request asks to create or evolve feature architecture docs, recommend `domainspec-orchestrate "architecture <feature>"` and include `domainspec-feature-architecture <feature>` as the advanced direct equivalent.
	- If the request asks to create or evolve a feature glossary, recommend `domainspec-orchestrate "glossary <feature>"` and include `domainspec-feature-glossary <feature>` as the advanced direct equivalent.
	- If the request asks to create or evolve implementation layering docs (POC-first rollout), recommend `domainspec-orchestrate "implementation layering <feature>"` and include `domainspec-implementation-layering <feature>` as the advanced direct equivalent.
