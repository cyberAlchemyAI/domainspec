...
1a. Apply delegation tuning + tracking for delegated stages in implementation flow (audits, context-builder, decision gate, gsd-phase bridge):
   ...
   - Append one telemetry entry per delegated stage to `docs/signals/delegation-tuning.jsonl` with profile, thinking budget, outcome, retries, and notes.
...
9. **Cross-feature stub prohibition:**
...
10. Run automated checks and report failures with remediation.
11. In --strict mode, stop on first doc-code mismatch and request spec fix.
</process>
