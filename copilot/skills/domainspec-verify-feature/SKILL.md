...
3a. **TEST-SPEC obligation coverage gate (MANDATORY):**
   ...
   b. Build covered obligation ID set by scanning executable test sources and automated test outputs for IDs embedded in test names (`KG-[A-Z0-9-]+`).
   ...
   e. Enforce severity:
      - Any uncovered `P0` ID from `Legacy Pipeline Must-Pass Subset` -> **BLOCK**.
