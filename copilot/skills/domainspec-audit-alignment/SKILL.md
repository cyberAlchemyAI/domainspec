...
4a. **Test obligation coverage gate (MANDATORY):**
   a. Parse `docs/features/{feature}/TEST-SPEC.md` `## Test Catalogue` and build the expected obligation ID set (for example `KG-*`).
   b. Build implemented-evidence ID set by scanning executable test sources and automated test outputs for obligation IDs in test names (`KG-[A-Z0-9-]+`).
   ...
   e. Severity policy:
      - Any uncovered ID in `Legacy Pipeline Must-Pass Subset` (`P0`) -> **BLOCK**.
