...
that shape normalizes. It is _not_ the field schema — the authoritative field-by-field reference
is owned by [`register-dispatch/SKILL.md`](skills/register-dispatch/SKILL.md), and the field
_semantics_ (the law) are owned by the
[subagents-strategy constitution, v0.9.0](../constitution/subagents-strategy-constitution-proposal.md) §5.
...
`<repo-root>/telemetry/agents/subagents-dispatch.yaml`. It answers: _who was dispatched, with
what intent, tensioned how, and how did it close._
...
- `internal_tools/agents-telemetry/` — a SQLite store of **usage measurement** (tokens,
  duration). Different question: _how much did it cost_, not _what was the design_.
...
1. a **dispatch row** — the spec, written _at dispatch_ (the human-confirmed sheet);
2. a **close row** — `close_of` + the outcome, written _at termination_.
...
by [SKILL.md](../skills/register-dispatch/SKILL.md) and the appender (v0.9.0 at time of writing).
...
dispatch row
  ...
  experiment_contract?  ← JSON column, required for v0.9 experiment rows
  │
  ├── groups[]          ← JSON column
...
`feedback_prompts`, and for v0.9 experiment rows the phase-matched `experiment_closeout`. The field rules — the `exit_reason` vocabulary, the `agents_spawned` shape,
...
`DISPATCH-COMPOSITION-MODEL` (the TO-VLAD6 observe-model: _flat append-only rows joined by FK,
not nested objects_). **The normalization is a design, not yet what the appender writes** — today
...
- _why_ the fan-out exists (`goal`, `context`),
...
worker, a triple of _authored intent · observed behaviour · execution evidence_ — complementary,
not overlapping.
...
the deliberate split: the hook guarantees the model is _reminded_; the model supplies the meaning
...
**The law (the canonical v0.9.0 proposal in `internal_tools/subagents-dispatch-hooks/constitution/`):**

- [subagents-strategy constitution, v0.9.0](../constitution/subagents-strategy-constitution-proposal.md)
  ...
  the _active_ `vault/constitution/` file is still v0.3.0 (rules `R1..R31`, a different
  vocabulary) and does **not** govern the v0.9.0 ledger — the v0.9.0 law lives in this bundle's
  single canonical `constitution/` folder.
...
- `experiment` — the LIVE type skill for the v0.9.0 propose/run experiment contract and exact criterion lineage.
- `other` — the LIVE bounded-execution fallback; every mutating lane requires an independent downstream reviewer.
