...
- **Silent ledger corruption** — the appender structurally self-checks the ledger before every append and refuses (exit 1) if it is corrupt, so damage surfaces at the next write instead of accumulating.
...
- **`skills/register-dispatch/`**: The skill that writes the ledger (installed to `~/.claude/skills`).
  - **[SKILL.md](skills/register-dispatch/SKILL.md)**: Owns the row schema (field reference, per-agent `angle` semantics), the close-event schema, and the authoring procedure (temp JSON file → appender).
  - **[append-dispatch.cjs](skills/register-dispatch/append-dispatch.cjs)**: The only sanctioned write path. Owns the appender contract: UTF-8 file argument (not stdin), idempotency on `dispatch_id`/`close_of`, JSON-column emission, repo-root resolution, and the pre-append structural self-check (rules documented in its header).
