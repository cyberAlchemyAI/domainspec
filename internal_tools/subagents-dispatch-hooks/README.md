...
| **Type skill** (LIVE: `research`, `review`, `experiment`, `other`) | [`skills/research/`](skills/research/SKILL.md) · [`skills/review/`](skills/review/SKILL.md) · [`skills/experiment/`](skills/experiment/SKILL.md) · [`skills/other/`](skills/other/SKILL.md) | type-specific judgment only — roles, tension axes, outputs, and closure. Defines no field. |
...
The **law** these point to lives in [`constitution/`](constitution/): field semantics (§5), the `dispatch_type` vocabulary, and the principles (P1–P12). `constitution/subagents-strategy-constitution-proposal.md` is the single canonical constitution (the repo-root copy was deleted) and is the v0.8.3 proposal that governs new rows (wire `schema_version` is `0.8.0`); `constitution/robot-talks-constitution.md` is the companion bound by robot-talks groups.

## Business Context

This repo runs multi-agent dispatches under a subagents-strategy constitution (wire schema v0.8.0): every dispatch contributes **two appends** — the dispatch row (the spec: `goal`, `context`, `groups` with per-agent `angle`/`model`/`token_budget`/`initial_prompt`, `connections`) at dispatch, and the close row (`close_of` + `exit_reason` + `agents_spawned`, optionally the verbatim `feedback_prompts`) at termination. Registration is a **skill the model invokes**, not an auto-logger, because a hook sees only the raw `Agent` call and cannot author `angle`/`anti_bias`/`goal`, and not every dispatch is a research dispatch with a roster to derive from. The reminder hook only nudges.
...
- **Silent ledger corruption** — the appender structurally self-checks the ledger before every append and refuses (exit 1) if it is corrupt, so damage surfaces at the next write instead of accumulating. Incoming records are additionally validated strictly against the v0.8.0 schema (exit 2 on violation; this includes the `anti_bias_global` conditional when ≥ 2 groups have ≥ 2 agents) — but **only the incoming record**: rows written under older schemas are grandfathered historical artifacts, never re-validated.
...
- **`skills/research/`, `skills/review/`, `skills/experiment/`, `skills/other/`**: The four LIVE **type skills**. Each owns only its type's judgment and points back to the router / form / constitution for everything universal. `other` is the bounded execution fallback and requires an independent reviewer for every mutating lane.
- **`skills/robot-talks/`**: The companion skill for intra-group discussion (`robot_talks: true`).
- **`constitution/`**: The **law** the skills point to — the single canonical location (the repo-root copy was deleted). `subagents-strategy-constitution-proposal.md` (v0.8.3 proposal — field semantics §5, `dispatch_type` vocabulary, principles P1–P14) and `robot-talks-constitution.md`.
