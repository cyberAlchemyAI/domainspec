...
3. If the request is DomainSpec natural language without an explicit command, map to one specialist workflow:
   ...
   - fourth-sibling typed ontology-view authoring (typed nodes + typed edges with forbidden-edge guards — forbidden relationships made unconstructible, not merely asserted) once a project carries the discovery / system-view / engineer-view triad (or a source corpus rich enough to mine) AND has a decision inventory for verdicts to point at -> `ontology-view <project-or-corpus-path> [--mode draft|validate|review|publish]`. Precondition: do NOT route here without the view triad/corpus and a decision inventory; recommend writing the discovery siblings first. This is a discovery-layer view, not an implementation-pipeline step.
