...
8. For each discovered feature, ensure a discovery artifact exists before authoring the spec (auto-create-placeholder-discovery step):
   - Check for an existing discovery in this order:
     1. `vault/discovery/<topic>-definitions/<slug>.md` (rare for brownfield — vault is reserved for framework discipline)
     2. `docs/features/<feature>/discovery/<slug>.md` (the application path — far more likely)
   - If a discovery exists at either location, proceed to step 9.
   - If MISSING, auto-create a placeholder at `docs/features/<feature>/discovery/<slug>.md` so brownfield translation does not stall on the discovery gate enforced by `domainspec-spec-feature` and the planner/verifier flow. Never write placeholders to `vault/**` — placeholders are application-scope by definition.
   - Placeholder frontmatter (use the project's standard frontmatter from `.claude/skills/custom/frontmatter.md`; key additions for placeholders):
     ```
     node_type: discovery
     status: placeholder
     ...
     feature: <feature>
     ```
   - Placeholder body (3–5 sections, all marked "TODO — human review"):
     - **Observed Behavior** — current behavior summarized from code/docs/tests (what the feature does today, observed not aspired).
     ...
     - **Promotion Path** — one line: "When the human reviews this and adds intent/rationale, change `status: placeholder` → `status: active` to promote to a full discovery."
   - After creating the placeholder, proceed to step 9. The placeholder satisfies the downstream discovery gate for spec-writing, planner, and verifier — but must never be silently promoted; promotion to `status: active` requires explicit human review.
   - Note: the interviewer's readiness summary distinguishes `status: placeholder` from `status: active` discoveries — placeholders are treated as a flagged-gap, not a passing artifact. (Forward-reference: the interviewer agent owns this distinction; this skill only emits the placeholder.)
9. For each discovered feature, create DomainSpec as-is docs under `docs/features/{feature}/`:
...
10. Produce brownfield alignment artifacts:
...
11. If `--audit-only` is provided, stop after Stage 1 and emit only research findings.
