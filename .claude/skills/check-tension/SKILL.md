...
**This skill owns the rubric**; nothing below points out to a doc for the test definitions.
(Tests 1–4 are constitution Principle 5's four tests; test 5 is the cross-group check. Setup —
identify subject groups, read each `angle`/`anti_bias`, classify the axis — is the
strategist's, done before the gate runs.)
...
- **Test 1 — axis.** `anti_bias` names one canonical axis above, or an explicitly declared
  composite of them. Outside the vocabulary → REJECT.
