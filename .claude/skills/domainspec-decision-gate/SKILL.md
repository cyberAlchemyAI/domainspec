...
4. Ask the user each decision:
   - Preferred: AskUserQuestion tool.
   - Fallback: plain conversation with numbered options if AskUserQuestion is unavailable.
...
7. Write the decision artifact with a table containing:
   ...
   - selected option
   - rationale
   - source (AskUserQuestion or chat fallback)
   - timestamp
