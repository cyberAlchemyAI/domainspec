...
1. Detect unified startpoint intent.
   ...
   - Delegate to `.claude/skills/domainspec-start/SKILL.md`.
...
4. Detect brownfield translation intent.
   ...
   - Delegate to `.claude/skills/domainspec-brownfield-translation/SKILL.md`.
...
5. Detect interview-kits intent.
   ...
   - Inspect `.claude/skills/domainspec-interview-kits/MODE-REGISTRY.md` to resolve the correct mode before invoking.
...
10. Return a readiness summary.
    ...
    - In brownfield mode, the Findings table MUST include a distinct row category for **discovery coverage per feature**, kept separate from missing specs/tests/docs rows. For each feature under `docs/features/{feature}/`, classify into one of three states using the existing `| Area | Status | Notes |` row shape:
      - `Discovery: <feature>` — Status `✅ has discovery` — Notes cite the discovery file path(s) under `docs/features/{feature}/discovery/`.
      - `Discovery: <feature>` — Status `⚠️ discovery_waived` — Notes cite the SPEC frontmatter `discovery_waived: true` and copy `discovery_waiver_reason`.
      - `Discovery: <feature>` — Status `❌ no discovery / no waiver` — Notes flag this as an audit gap.
    - These rows are surfaced as soft signals; they do not block the readiness summary.
      </execution>
...
| Area                 | Status                                                                 | Notes                                                                                                                                       |
| -------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| Discovery: <feature> | ✅ has discovery / ⚠️ discovery_waived / ❌ no discovery and no waiver | path(s) to discovery file under `docs/features/{feature}/discovery/`, or `discovery_waiver_reason` from SPEC frontmatter, or audit-gap note |
...
If Step 1 delegated to unified startpoint, return the structured summary defined by `.claude/skills/domainspec-start/SKILL.md`.
If Step 4 delegated to brownfield translation, return the structured summary defined by `.claude/skills/domainspec-brownfield-translation/SKILL.md`.
