...
Template: [templates/domainspec-findings.md](../../templates/domainspec-findings.md).
Constitution rules to honor: R16 (three sections in order), R17 (citation requirement), R18 (Dispatch record schema), R21+R22 (four-component grade with judgment markers), R23 (Context + Goal preamble).
</context>
...
4. Write `<working_folder>/research/domainspec-findings.md` with:
   ...
   - **Dispatch record** section (R18) — mode, per-agent table, sequencing, recursion budget actually used, actual spend table, four-component grade per R21/R22:
     - **Coverage** (judgment) — score against the Goal.
     - **Independence** (judgment) — score on concern non-overlap.
...
<output>
Single line confirmation with grade summary as above.

If a constraint is violated (vault path, missing R18 field, claim without citation, missing judgment marker): return the specific violation and stop. Do not write a partial file.
</output>