...
agents:
  - id: e1
    ...
    axes:
      {
        methodology: formal,
        source_corpus: arxiv-categorical,
        temporal_prior: modern,
      }
  - id: e2
    ...
    axes:
      {
        methodology: formal,
        source_corpus: physics-journals,
        temporal_prior: historical,
      }
  - id: e3
    ...
    axes:
      {
        ...
        source_corpus: citation-tree,
        temporal_prior: mixed,
      }
```
...
All three reach the same bottom-line ("the PR is good"). The post-dispatch validator (Item 10) flags this as a candidate false consensus and prompts each skeptic with a layer-specific tension prompt: "What is the strongest argument the _other two_ skeptics could raise against your finding?"
...
The wrong response is to take the unanimous "yes, ship it" as five times more evidence than a single agent's "yes". Correlated unanimity is _not_ multiplicative evidence. Treating it as such is the failure mode the principle exists to prevent.
