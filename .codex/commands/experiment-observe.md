...
- `run_id`: use an existing hook-provided run id when present; otherwise use `arcanum-experiment-observe-<UTC timestamp>`.
...
Run Experiment Harness in `observe` mode.

## Command Shape

```text
/experiment-observe <artifact-path> [report-path]
```
...
3. Run or explain the equivalent script operation for `observe` mode.
...
- Keep this adapter focused on Experiment Harness `observe` mode.
