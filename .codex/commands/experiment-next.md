...
- `run_id`: use an existing hook-provided run id when present; otherwise use `arcanum-experiment-next-<UTC timestamp>`.
...
Run Experiment Harness in `next` mode.

## Command Shape

```text
/experiment-next <artifact-path> 
```
...
3. Run or explain the equivalent script operation for `next` mode.
...
- Keep this adapter focused on Experiment Harness `next` mode.
