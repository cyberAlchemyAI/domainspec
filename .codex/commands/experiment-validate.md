...
- `run_id`: use an existing hook-provided run id when present; otherwise use `arcanum-experiment-validate-<UTC timestamp>`.
...
Run Experiment Harness in `validate` mode.

## Command Shape

```text
/experiment-validate <artifact-path> 
```
...
3. Run or explain the equivalent script operation for `validate` mode.
...
- Keep this adapter focused on Experiment Harness `validate` mode.
