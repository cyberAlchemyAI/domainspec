...
<!-- arcanum:command arcanum-spell-necronomicon -->
...
- `run_id`: use an existing hook-provided run id when present; otherwise use `arcanum-arcanum-spell-necronomicon-<UTC timestamp>`.
...
Run the installed Arcanum spell `necronomicon` using the canonical spell snapshot embedded below.
...
- Keep this command focused on `necronomicon`.
