...
- `run_id`: use an existing hook-provided run id when present; otherwise use `arcanum-experiment-run-<UTC timestamp>`.
...
Run Experiment Harness in `run` mode.

## Command Shape

```text
/experiment-run <artifact-path> <example-id|next|--all>
```
...
3. Run or explain the equivalent script operation for `run` mode.
...
- Keep this adapter focused on Experiment Harness `run` mode.
