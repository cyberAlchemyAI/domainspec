...
- Invoke the `domainspec-implementation-axioms` skill before implementing. The 4 axioms (AX-DS-1..4) it carries are non-negotiable; every execution step below is justified by at least one of them. The `<axioms>` block below is a summary — the skill is authoritative.
...
<axioms>
Authoritative source: `domainspec-implementation-axioms` skill (invoke before any execution step). Summary of step↔axiom mapping below.

- **AX-DS-1 — Spec is source; code is its compiled image.** Defended by steps 1–2, 6 (build from docs) and step 3 (alignment audit).
- **AX-DS-2 — One vocabulary across spec and code.** Defended by step 8 (tag-code) and the registry sync that follows.
- **AX-DS-3 — No orphan behavior.** Defended by step 3 (layering audit), step 7 (tests linked to source clauses), and step 8 (tagging).
- **AX-DS-4 — Decision space is preserved with the decision.** Defended by step 4 (baseline interview gate).

Violation handling: see the `<violation-handling>` block in the `domainspec-implementation-axioms` skill. Stop and surface conflicts by axiom ID — never proceed with a silent compromise.
</axioms>
...
8. Delegate to `domainspec-code-tagger` (or run `domainspec-tag-code <feature> --mode strict`) after code edits.
9. Run automated checks and summarize results with traceability.
10. **Emit signals** — follow `.github/skills/domainspec-emit-signals/SKILL.md` to append any alignment gaps, spec gaps, rework, decisions, or patterns discovered during implementation to `docs/signals/pipeline-signals.jsonl`.
</execution>
