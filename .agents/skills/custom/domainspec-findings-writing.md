...
- **Agreements** - claims that multiple explorers' research independently supports (cite 2 or more explorers; qualify as `strong` / `majority` / `partial`).
...
- **Recommended next action** - promote as a discovery, re-dispatch with adjusted strategy, or hold.
...
- [ ] Claims are grounded in `research.md`, not introduced from nowhere.
