...
The skill organizes a dispatch into layers (explorer, skeptic, evaluator, synthesizer, writer, auditor). The principle applies *layer-locally*: it constrains the choice of angles for the agents within a single layer of size N ≥ 2, not the relationship between layers.
...
- **Evaluator layer.** N parallel agents scoring an artifact against criteria. Angles must be tensioned along criterion family or stake. Bad: three "rate this 1-10" agents. Good: rigor-evaluator + audience-fit-evaluator + opportunity-cost-evaluator (what is this PR displacing?).
