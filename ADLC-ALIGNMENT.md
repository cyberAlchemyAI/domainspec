...
| M-006 | **Overhead Ratio**    | Governance cost relative to domain work                                | Economy of Action counters (G7) | Not tracked                 |
...
| G2  | **Continuous tuning outer loop**   | Phase 5 — Monitoring is the value engine                      | Production data exists (OTel, SLOs) but doesn't feed back to agent improvement | Closed loop: production metrics → identify tuning opportunities → update prompts/skills → re-evaluate                                                     |
...
| G4  | **Automated governance**           | Value 4 — Automated governance over manual management         | Alignment/layering audits are manual triggers                                  | Continuous automated governance: audits run on every commit, drift alerts auto-created, compliance dashboards                                             |
...
| G7  | **Economy of Action metrics**      | Principle 10 — Minimize computational and cognitive load      | Context discovery heuristic exists but unmeasured                              | Track and report: tokens consumed, agent calls made, human questions asked, time-to-verdict per feature                                                   |
| G8  | **Reflection and auto-tuning**     | Principle 12 — System reflects and tunes at every interaction | PASS/FLAG/BLOCK is a one-shot verdict                                          | After each pipeline run, agent analyzes what worked/failed, proposes prompt/skill improvements, and logs them for review                                  |
...
timeline
    ...
    v2.0 — Tuning        : G2 Closed-loop outer tuning cycle
...
| T3   | backend | Instrument pipeline runs with Economy of Action counters: total tokens, agent invocations, human prompts, wall-clock time                                                                                                                                                                                                                                                                    |
| T4   | docs    | Add Economy of Action section to OBSERVABILITY-REPORT.md template                                                                                                                                                                                                                                                                                                                            |
...
| T19  | framework | Add reflection step to pipeline: after PASS/FLAG/BLOCK, agent writes a structured retrospective (what went well, what was ambiguous, suggested improvements)                   |
