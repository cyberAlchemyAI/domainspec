...
import type { ProjectSourceRegistryPort } from "../application/ports.js";
...
export function createInMemoryProjectSourceRegistry(
  ...
  return {
    resolveProjectionScope(input) {
      ...
      if (
        !isPathWithin(source.workspaceRootDir, source.relationshipsFilePath)
      ) {
        throw createKnowledgeGraphError(
          "MIRROR_SOURCE_ROOT_INVALID",
