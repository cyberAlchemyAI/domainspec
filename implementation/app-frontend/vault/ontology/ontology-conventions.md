...
This document specifies the exact frontmatter fields, the classification system (node types, scopes, representation layers, natures, tags), the confidence dimensions, and the full catalog of edge types with their directionality rules.
...
3. [`scope` — System Scope](#scope--system-scope)
4. [`representation_layer` — Meta-Representation Level](#representation_layer--meta-representation-level)
5. [`nature` — Document Format](#nature--document-format)
6. [`status` — Maturity Level](#status--maturity-level)
7. [`veracidade` and `convicção` — The Two Dimensions of Confidence](#veracidade-and-convicção--the-two-dimensions-of-confidence)
8. [`tags` — Domain Keywords](#tags--domain-keywords)
9. [Edge Types (Connections Section)](#edge-types-connections-section)
10. [The Orthogonality Principle](#the-orthogonality-principle)
11. [Open Questions](#open-questions)
12. [Appendix A: Mathematical Foundation](#appendix-a-mathematical-foundation)
13. [Appendix B: Label Value Catalog](#appendix-b-label-value-catalog)
14. [Appendix C: Edge Type Catalog](#appendix-c-edge-type-catalog)
15. [Appendix D: Quick Reference — The 8 Labels](#appendix-d-quick-reference--the-8-labels)
...
node_type: axiom | premise | constitution | discovery | implementation-plan | spec | audit | conceptual | essay | test | backlog | readme | agent-dialogue
...
scope: ontology | architecture | market | domain | application   # Multi-value allowed
representation_layer: L0 | L1-operational | L1-architectural | L2 | L3 | L4 | L5 | L6 | L7  # Multi-value allowed
representation_track: object | meta | emergent | mixed          # Optional helper (derived from representation_layer)
...
| `essay`               | "It's a committed argument — engage with it or counter it with a better one" |
...
| `agent-dialogue`      | "Treat it as provenance from agent collaboration — synthesize and promote durable decisions into constitutions/specs if needed" |
...
The thirteen `node_type` values — `axiom`, `premise`, `constitution`, `discovery`, `implementation-plan`, `spec`, `audit`, `conceptual`, `essay`, `test`, `backlog`, `readme`, `agent-dialogue` — each represent a distinct role with precise boundaries. For the full value definitions and differentiation criteria, see [Appendix B: Label Value Catalog](#appendix-b-label-value-catalog).
...
> **Why `session` is not a node_type (and when to use `agent-dialogue`):** `session` is too generic and loses role information. Use `agent-dialogue` only for conversation artifacts where the dialogue itself is the primary output to preserve. If the durable output is a rule, plan, or behavior description, classify by role (`constitution`, `implementation-plan`, `spec`, etc.) and keep conversation provenance in `is_session`/`session_ref`.
...
> **Why `business` is not a node_type:** A document about the market plays the same role as any other `conceptual` document — background context with no enforcement power. The market/external scope is captured by `scope: market`. Using `node_type: business` would correlate almost perfectly with `scope: market`, violating orthogonality.
...
`scope` classifies **what part of the system or company** the document concerns. It is a topical boundary — not an epistemic level, not a representation level, not a format.

### Why it matters

Without `scope`, an agent searching for "all architecture rules" would have to read every document's content to determine if it's about architecture. With `scope: architecture`, it's a single `WHERE` clause. This is the primary filter for narrowing search space.

### How it differs from `node_type`

`node_type` and `scope` are independent axes. An axiom can be about architecture or the market. A constitution can be about architecture or the ontology. A conceptual document can live in any scope. Knowing the `node_type` tells you nothing about the `scope`, and vice versa.

### Multi-value scope

A document **may belong to more than one scope**. For example, a session that discussed both architecture refactoring and market reality can be `scope: architecture, market`. Use multi-value when a document genuinely spans multiple scopes. **Do not use a special value to indicate multi-scope documents — just list the scopes.**

The five `scope` values — `ontology`, `architecture`, `market`, `domain`, `application` — each scope a different part of the system. For the full value definitions and examples, see [Appendix B: Label Value Catalog](#appendix-b-label-value-catalog).

> **Why `session` is not a scope:** A session about architecture should be `scope: architecture`, not `scope: session`. The `is_session` flag already tells you it's a conversation log — making `scope: session` redundant and losing the information about *what the session was about*. The `scope` should always reflect the document's topic, not its format.

> **Why `cross` was removed:** `cross` was a special value meaning "this spans multiple scopes" — but that's exactly what multi-value syntax expresses directly and more informatively. `scope: architecture, domain` is strictly better than `scope: cross` because it tells you *which* scopes are involved.

---

## `representation_layer` — Meta-Representation Level

### What it is

`representation_layer` classifies **which level of representation** the document primarily operates on in the L0-L7 model:

- `L0`: domain reality
- `L1-operational`: practitioner understanding
- `L1-architectural`: formalized domain understanding
- `L2`: implementation artifacts (code/contracts/schema)
- `L3`: governing rules
- `L4`: epistemic foundations
- `L5`: knowledge navigation
- `L6`: enforcement
- `L7`: emergent semantic coherence

### Why it matters

Without `representation_layer`, two documents may look similar by scope but play very different roles in the representation stack. Example: both can have `scope: domain`, while one is `L1-architectural` (formal model) and the other is `L2` (runtime implementation contract).

### How it differs from `scope`

`scope` answers *where* the document applies. `representation_layer` answers *at what representational level* it operates. They are orthogonal: a document can be `scope: architecture` and `representation_layer: L3`, or `scope: domain` and `representation_layer: L2`.

### Multi-value representation layer

Use multi-value only when the document genuinely spans levels (for example, migration docs that intentionally bridge `L1-architectural` and `L2`). Otherwise prefer one dominant level.

### `representation_track` (derived helper)

`representation_track` is a convenience projection, not a core orthogonal label:

- `object` = L0-L2
- `meta` = L3-L6
- `emergent` = L7
- `mixed` = any intentional cross-track combination

Because `representation_track` is computed from `representation_layer`, it is optional and should not be used in orthogonality metrics.
...
The 8 core classification labels (`node_type`, `scope`, `representation_layer`, `nature`, `status`, `veracidade`, `convicção`, `tags`) are designed so that **knowing the value of one label gives you no information about the value of any other**. This is what makes each label worth maintaining — it captures a dimension of meaning that would be lost without it.
...
**Context:** `scope` captures broad boundary (architecture, market, domain), but NOT the specific business domain. A spec about aquisição and a spec about liquidação both have `scope: domain` — you need tags (`#acquisition`, `#liquidation`) to tell them apart. Should there be a dedicated `domain` field that captures the specific domain?

**Arguments for:** Tags are free-text and unenforced. A formal `domain` field would be required and validated. For specs, the domain is critical metadata — arguably more important than the broad scope.
...
Let the classification system consist of *n* labels: **L₁, L₂, ..., Lₙ** (currently *n = 8* core labels). Each label is a discrete random variable whose value is drawn from a finite set (e.g., `node_type ∈ {axiom, premise, constitution, discovery, implementation-plan, spec, audit, conceptual, essay, test, backlog, readme, agent-dialogue}`).
...
| `readme`              | Navigation document for a directory. Summarizes structure and points to canonical files so readers can find authoritative content quickly.                                                                                                                 | "Update it to reflect what's actually in the directory"                  | `README.md`                                                                 |
| `agent-dialogue`      | Structured record of multi-agent discussion, proposals, tensions, and synthesis. Preserves reasoning lineage; durable decisions should be promoted to constitution/spec/plan nodes.                                                                        | "Use it as provenance and synthesis context, then promote stable outputs" | `robot-talk` session logs                                                   |

### `scope` Values

| scope | What it scopes | Example documents |
|-------|---------------|-------------------|
...
For documents that genuinely span multiple scopes, use multi-value: `scope: architecture, domain`. Do not use a special value — list the scopes explicitly so the information is preserved.

### `representation_layer` Values

| representation_layer | What it captures | Example documents |
|----------------------|------------------|-------------------|
| `L0` | Domain reality prior to software representation. Mostly contextual anchors and field observations. | Market/domain observation notes |
| `L1-operational` | Practitioner tacit operating model and exception handling knowledge. | Operator playbooks, workflow notes |
| `L1-architectural` | Formalized domain understanding (concept model/spec vocabulary) independent from implementation details. | Domain specs, concept dictionaries |
| `L2` | Executable representation: code contracts, schemas, integrations, and implementation-facing specs. | API contracts, schema docs, implementation specs |
| `L3` | Governing rules that prescribe behavior and constraints. | Constitutions, conventions, operational skills |
| `L4` | Epistemic foundations backing L3 rules. | Axioms, premises |
| `L5` | Navigation and retrieval structures over the knowledge graph. | Navigation maps, index topology docs |
| `L6` | Enforcement mechanisms turning rules into blockers/checks. | CI gates, validation pipeline specs |
| `L7` | Emergent semantic coherence across aligned layers. | System-level coherence assessments |

Use multi-value only for intentional bridge documents spanning adjacent levels.

### `representation_track` Mapping (Optional)

`representation_track` is a convenience field derived from `representation_layer`:

- `object`: L0-L2
- `meta`: L3-L6
- `emergent`: L7
- `mixed`: cross-track documents

Because it is derived, it is optional and excluded from orthogonality accounting.
...
## Appendix D: Quick Reference — The 8 Labels

Every vault document carries up to 8 core classification labels. Each answers a different question. If two labels answered the same question, one would be redundant.

| Label | Question | What it captures | Independent of |
|-------|----------|-----------------|----------------|
| **`node_type`** | *What role does this document play?* | Kind of claim: axiom, premise, constitution, discovery, implementation-plan, spec, audit, conceptual, essay, test, backlog, readme, agent-dialogue | All others — an axiom can be about any scope, any representation layer, any nature, any status |
| **`scope`** | *What part of the system/company does it concern?* | Topical boundary: ontology, architecture, market, domain, application | `node_type` (a constitution can be about architecture or market), `representation_layer` (where ≠ representational level), `nature` (scope ≠ format) |
| **`representation_layer`** | *At what representation/control level does it operate?* | L0-L7 stack position from domain reality to emergent coherence | `scope` (same level can occur in different scopes), `node_type` (same role can live at different levels) |
| **`nature`** | *What structural format does it use?* | Reading instruction: explanatory prose, step-by-step, lookup table, or schema | `node_type` (a constitution can be a checklist or a schema), `scope` (format ≠ topical boundary), `representation_layer` (format ≠ level) |
...
> **Why not fewer labels?** If we merged `node_type` and `status` into one dimension, we couldn't distinguish "this is an axiom in draft" from "this is a premise that's consolidated." If we merged `scope` and `representation_layer`, we couldn't distinguish "a domain document at L1-architectural" from "a domain document at L2." If we merged `scope` and `nature`, we couldn't distinguish "a market document written as a reference table" from "a market document written as a how-to." Each label captures information that NO other label can express.
