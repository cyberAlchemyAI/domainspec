...
operations brief such as _"clinic dashboard", "doctors schedule", "hospital
admin", "appointment console", "telemedicine ops", "诊所后台", "医院管理"_.
...
```json
{
  ...
  "sources": [{ "label": "Template default sample", "type": "user_input" }]
}
```
...
> didn't, the validator runs _before_ substitution, so a malformed `data.json`
...
The renderer's security validator runs _before_ `{{data.*}}` substitution, so
