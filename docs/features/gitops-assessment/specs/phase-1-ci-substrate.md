...
| Vault edge resolver | `tools/validate-vault-edges.ts` | TS script that parses the `## Connections` table in every changed `vault/**/*.md` (strict markdown table parser per Appendix C format), fails if any link target does not exist on disk. | Phase 1 author |
...
| Pre-commit extension | `.githooks/pre-commit` (modified) | Adds local invocation of `tools/validate-vault-frontmatter.ts` and `tools/validate-vault-edges.ts` for staged `vault/**` files (fast feedback before push). | `.githooks/` owner |
