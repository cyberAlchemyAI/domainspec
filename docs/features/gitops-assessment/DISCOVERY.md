...
- `tools/validate-vault-frontmatter.ts`, `tools/validate-domainspec-vault-edges.ts`, `tools/surface-vault-contradicts.ts`, `tools/check-vault-status-promotion.ts`, `tools/check-vault-broken-links.ts` — the deterministic vault validators (new code; ~5 scripts).
...
| **`tools/validate-domainspec-vault-edges.ts`** *(new, v1)* | PR touching `vault/**/*.md` | `pr-validate.yml` | edge type not in Appendix C catalog, target file does not resolve, or bidirectional-pair declaration mismatch blocks merge |
